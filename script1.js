document.addEventListener("DOMContentLoaded", function () {
    // Typing effect
    const typedText = "Hi Welcome, I'm Daniel Muuo Muumbi, a Fullstack Developer. I create sophisticated software solutions with a touch of innovation.";
    let index = 0;
    const typingSpeed = 50;
    
    function typeText() {
        if (index < typedText.length) {
            document.getElementById("typed-text").innerHTML += typedText.charAt(index);
            index++;
            setTimeout(typeText, typingSpeed);
        } else {
            setTimeout(() => {
                document.getElementById("typed-text").innerHTML = "";
                index = 0;
                typeText();
            }, 2000);
        }
    }

    typeText();

    // Toggle links on arrow or link click
    const arrow = document.querySelector(".arrow-down");
    const links = document.querySelector(".pages-links");

    function toggleLinks() {
        links.style.display = links.style.display === "flex" ? "none" : "flex";
    }

    arrow.addEventListener("click", toggleLinks);

    links.querySelectorAll("a").forEach(link => {
        link.addEventListener("click", toggleLinks);
    });

    // Initialize particles.js
    particlesJS('particles-js', {
      "particles": {
        "number": {
          "value": 80,
          "density": {
            "enable": true,
            "value_area": 800
          }
        },
        "color": {
          "value": "#ffffff"
        },
        "shape": {
          "type": "circle",
          "stroke": {
            "width": 0,
            "color": "#000000"
          },
          "polygon": {
            "nb_sides": 5
          }
        },
        "opacity": {
          "value": 0.5,
          "random": false
        },
        "size": {
          "value": 3,
          "random": true
        },
        "line_linked": {
          "enable": true,
          "distance": 150,
          "color": "#ffffff",
          "opacity": 0.4,
          "width": 1
        },
        "move": {
          "enable": true,
          "speed": 6,
          "direction": "none",
          "random": false,
          "straight": false,
          "out_mode": "out"
        }
      },
      "interactivity": {
        "detect_on": "canvas",
        "events": {
          "onhover": {
            "enable": true,
            "mode": "repulse"
          },
          "onclick": {
            "enable": true,
            "mode": "push"
          },
          "resize": true
        },
        "modes": {
          "grab": {
            "distance": 400,
            "line_linked": {
              "opacity": 1
            }
          },
          "bubble": {
            "distance": 400,
            "size": 40,
            "duration": 2,
            "opacity": 8,
            "speed": 3
          },
          "repulse": {
            "distance": 200
          },
          "push": {
            "particles_nb": 4
          },
          "remove": {
            "particles_nb": 2
          }
        }
      },
      "retina_detect": true
    });

    // Dark mode toggle
    const modeIcon = document.getElementById('mode-icon');
    modeIcon.addEventListener('click', function () {
        document.body.classList.toggle('light-mode');
        modeIcon.textContent = document.body.classList.contains('light-mode') ? '🌞' : '🌙';
    });
});


